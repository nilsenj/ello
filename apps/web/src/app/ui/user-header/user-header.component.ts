...
@Component({
  ...
  template: `
    <header
      class="native-safe-header w-full relative z-50 bg-[var(--board-header,#026AA7)] text-white"
    >
      <div
        class="mx-auto max-w-full px-2 sm:px-4 py-2 flex items-center justify-between relative min-h-[52px]"
      >
        <!-- Left: Logo & Board Switcher -->
        <div
          class="flex items-center gap-1.5 sm:gap-3 min-w-0 flex-1"
        ...
        >
          ...
          >
            <!-- Logo Icon -->
            <div
              class="relative w-8 h-8 flex items-center justify-center bg-white/20 rounded-lg group-hover:bg-white/30 transition-colors backdrop-blur-sm shadow-sm ring-1 ring-white/20"
            >
              <svg
                width="20"
                height="20"
            ...
            <span
              class="hidden lg:block text-2xl font-black tracking-tighter text-white drop-shadow-sm group-hover:opacity-90 transition-opacity"
        ...
        <div
          ...
          [ngClass]="{
            'absolute inset-0 bg-[var(--board-header,#026AA7)] z-20 px-4 justify-center':
              mobileSearchOpen(),
        ...
        >
          ...
          <button
            *ngIf="mobileSearchOpen()"
            ...
            class="text-white mr-2 md:hidden"
          >
            <svg
              width="20"
              height="20"
          ...
          <div class="flex-1 w-full max-w-[400px] relative">
            ...
            <!-- Results Dropdown -->
            <div
              *ngIf="query().trim().length > 0 && showResults()"
            ...
            >
              ...
              <div *ngFor="let group of filteredBoards()">
                <div
                  class="px-3 py-2 text-xs font-semibold text-gray-500 bg-gray-50 uppercase tracking-wider"
                >
                  {{ group.workspaceName }}
                </div>
                <button
                  *ngFor="let board of group.boards"
                ...
                >
                  <div
                    class="w-4 h-4 rounded-sm bg-gray-200 shrink-0"
                    [style.background]="getBoardColor(board.background)"
                  ></div>
                  {{ board.name }}
                </button>
              </div>
        ...
        <div
          ...
          [class.hidden]="mobileSearchOpen()"
        >
          <!-- Voice Assistant Trigger (HIDDEN per user request) -->
          <button
            *ngIf="false"
            class="pill p-1.5 sm:p-2 text-indigo-100 hover:text-white hover:bg-white/20 transition-colors rounded-md shrink-0 flex items-center justify-center relative group"
            (click)="voiceOverlay.open()"
          >
            <svg
            ...
            <span
              class="absolute -bottom-8 right-0 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none"
          ...
          <button
            class="md:hidden pill p-1.5 sm:p-2 text-white hover:bg-white/20 transition-colors rounded-md shrink-0"
            (click)="toggleMobileSearch()"
          >
            <svg
              width="20"
...
export class UserHeaderComponent {
  ...
  initials = () =>
    (this.user()?.name || this.user()?.email || "U")
      .trim()
      .slice(0, 2)
      .toUpperCase();
  ...
  boardsByWorkspace = computed(() => {
    ...
    const fulfillmentBoards = boards.filter(
      (b) => b.type === "ecommerce_fulfillment",
    );
    const visibleBoards = isServiceDeskView
      ? serviceDeskBoards.length
        ? serviceDeskBoards
        : boards
      : isFulfillmentView
        ? fulfillmentBoards.length
          ? fulfillmentBoards
          : boards
        : boards;
    ...
    const otherBoards = visibleBoards.filter(
      (b) => !workspaces.find((w) => w.id === b.workspaceId),
    );
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (
        this.auth.isAuthed() &&
        !this.initialBoardsFetch &&
        this.store.boards().length === 0
      ) {
