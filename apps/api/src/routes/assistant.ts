...
function fuzzyMatch(dbName: string, spokenName: string): boolean {
  const cleanDb = dbName
    .toLowerCase()
    .replace(/^(the|a|my)\s+/i, "")
    .replace(/[.!?]+$/, "")
    .trim();
  const cleanSpoken = spokenName
    .toLowerCase()
    .replace(/^(the|a|my)\s+/i, "")
    .replace(/[.!?]+$/, "")
    .trim();
  return (
    cleanDb === cleanSpoken ||
    cleanDb.includes(cleanSpoken) ||
    cleanSpoken.includes(cleanDb)
  );
}
...
export const assistantRoutes: FastifyPluginAsync<{
  ...
  fastify.post("/api/assistant/transcribe", async (request, reply) => {
    ...
    if (!openai) {
      return reply
        .code(500)
        .send({ error: "OpenAI API Key is missing. Transcription disabled." });
    }
  ...
  fastify.post("/api/assistant/command", async (request, reply) => {
    ...
    if (command.action === "UNKNOWN") {
      return reply.code(400).send({
        ...
        details:
          'Ensure you are using the supported command formats or natural language like "Create board [name]".',
      });
    }
    ...
    try {
      ...
      switch (command.action) {
        ...
        case "CREATE_LIST": {
          ...
          const board = boards.find((b) =>
            fuzzyMatch(b.name, command.payload.boardName),
          );
          ...
          if (!board) {
            return reply
              .code(404)
              .send({
                error: `Board "${command.payload.boardName}" not found.`,
              });
          }
        ...
        case "CREATE_CARD": {
          ...
          if (command.payload.listName) {
            ...
            const list = lists.find((l) =>
              fuzzyMatch(l.name, command.payload.listName!),
            );
            ...
            if (!list) {
              return reply
                .code(404)
                .send({
                  error: `List "${command.payload.listName}" not found.`,
                });
            }
            listId = list.id;
          } else {
            ...
            if (!boardId) {
              return reply
                ...
                .send({
                  error:
                    "Active board context required to implicitly create a task.",
                });
            }
            ...
            if (!firstList) {
              return reply
                .code(400)
                .send({
                  error: "No lists found in active board to add the task to.",
                });
            }
            listId = firstList.id;
          }
      ...
      return reply
        .code(500)
        .send({ error: "Internal Server Error executing command." });
    }
  });
};
